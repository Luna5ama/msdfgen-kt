...
object EdgeColoring {
    ...
    private val INITAL_COLORS = EdgeColor.Array.of(EdgeColor.CYAN, EdgeColor.MAGENTA, EdgeColor.YELLOW)

    private fun isCorner(aDir: Vector2, bDir: Vector2, crossThreshold: Float): Boolean {
        val v = dotProduct(aDir, bDir) <= 0 || abs(crossProduct(aDir, bDir)) > crossThreshold
        return v
    }

    private fun estimateEdgeLength(edge: EdgeSegment): Float {
        var len = 0.0f
    ...
    private fun switchColor(
        ...
        if (color == EdgeColor.BLACK || color == EdgeColor.WHITE) {
            colors[i] = INITAL_COLORS[Math.floorMod(seed, 3).toInt()]
            seed /= 3
            return
        }
    ...
    fun edgeColoringSimple(shape: Shape, angleThreshold: Float = 3.0f, seedIn: Long) {
        ...
        for (contour in shape.contours) {
            ...
            if (edges.isNotEmpty()) {
                var prevDirection = edges.last().direction(1.0f).normalize()
                ...
                1 -> {
                    ...
                    if (edges.size >= 3) {
                        ...
                        for (i in 0 until m) {
                            ...
                            edges[(corner + i) % m].color = colors[colorIndex]
                        }
    ...
    //TODO: edgeColoringInkTrap

    //TODO: edgeColoringByDistance
}