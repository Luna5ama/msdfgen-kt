...
class Contour {
     ...
     */
    val edges = mutableListOf<EdgeHolder>()

    /**
     * Adds an edge to the contour.
     */
    fun addEdge(edgeHolder: EdgeHolder) {
        edges.add(edgeHolder)
    }

    /**
     * Creates a new edge in the contour and returns its reference.
     */
    fun addEdge(): EdgeHolder {
        val element = EdgeHolder()
        edges.add(element)
        return element
    }
    ...
    fun boundMitered(bound: Bound, border: Float, miterLimit: Float, polarity: Int) {
        ...
        var prevDir = edges.last().get().direction(1.0f).normalize(true)
        for (edge in edges) {
            val dir = -edge.get().direction(0.0f).normalize(true)
            if (polarity * crossProduct(prevDir, dir) >= 0.0f) {
                ...
                val miter = edge.get().point(0.0f) + border * miterLength * (prevDir + dir).normalize(true)
                boundPoints(bound, miter)
            }
            prevDir = edge.get().direction(1.0f).normalize(true)
        }
    }
    ...
    fun winding(): Int {
        ...
        when (edges.size) {
            1 -> {
                val a = edges[0].get().point(0.0f)
                val b = edges[0].get().point(1.0f / 3.0f)
                val c = edges[0].get().point(2.0f / 3.0f)
                total += shoelace(a, b)
                total += shoelace(b, c)
                total += shoelace(c, a)
            }
            2 -> {
                val a = edges[0].get().point(0.0f)
                val b = edges[0].get().point(0.5f)
                val c = edges[1].get().point(0.0f)
                val d = edges[1].get().point(0.5f)
                total += shoelace(a, b)
                total += shoelace(b, c)
                total += shoelace(c, d)
                total += shoelace(d, a)
                ...
                var prev = edges.last().get().point(0.0f)
                for (edge in edges) {
                    val cur = edge.get().point(0.0f)
